...
void rand_to_ppm(const int dim_x, const int dim_y)
{
    ...
    for(j=0; j<dim_y; ++j)
    {
        for(i=0; i<dim_x; ++i)
        {
            ...
            color[0]=rand()%255; /* red */
            color[1]=rand()%255; /* green */
            color[2]=rand()%255; /* blue */
            fwrite(color,1,3,fp);
        }
    }
    fclose(fp);
...
int main(int argc, const char **argv)
{
    ...
    else if (argc > 1)
    {
        ...
        else if(!strcmp(argv[1], "middle_square_method"))
        {
            middle_square_to_ppm(255, 255);
        }
        else    // TODO: Delete at the end or do somethign else with it
        {
            middle_square_to_ppm(255, 255);
        }
    }
